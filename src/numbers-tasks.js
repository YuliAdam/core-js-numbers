...
function getAverage(value1, value2) {
  ...
  if (res === Infinity) return +1.7976931348623157e308;
  if (res === -Infinity) return -1.7976931348623157e308;
  return res;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  let sub = res.substring(0, res.length - pow);
  if (Number(res[res.length - pow]) >= 5) sub = (Number(sub) + 1).toString();
  sub += '0'.repeat(pow);
  return Number(sub);
}
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(res) || res === undefined) res = def;
  return res;
}
